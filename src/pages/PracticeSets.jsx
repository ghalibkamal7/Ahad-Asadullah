...
import { ClipboardList, Eye, Download, X, Lock } from 'lucide-react'
...
export default function PracticeSets() {
  ...
  const { data: sets, loading } = useCollection(COLLECTIONS.PRACTICE_SETS, [orderByField('createdAt')])

  function isUnlocked(s) {
    return s.free || profile?.purchasedItems?.includes(s.id)
  }

  return (
    <div className="section py-14">
      <span className="eyebrow">Print & practice</span>
      <h1 className="text-3xl sm:text-4xl font-bold mt-2 mb-3">Practice Sets</h1>
      <p className="text-ink-500 dark:text-ink-400 max-w-xl mb-10">
        Subject-wise practice papers — PDF only, view or download and solve at your own pace.
      </p>
      ...
      {loading ? (
        ...
        <EmptyState icon={ClipboardList} title="No practice sets yet" text="Add practice sets from the Admin Panel." />
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {sets.map((s, i) => {
            ...
            return (
              ...
              >
                ...
                <p className="text-xs text-ink-400 mb-5">{s.subject || 'General'} · {s.free ? 'Free' : `₹${s.price || 0}`}</p>
                <div className="flex gap-2">
                  <button
                    onClick={() => unlocked && setPreview(s)}
                    disabled={!unlocked}
                  ...
                  {unlocked && s.fileUrl && (
                    <a href={s.fileUrl} target="_blank" rel="noreferrer" className="btn-primary flex-1 !py-2 text-sm">
